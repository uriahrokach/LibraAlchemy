...
def create_potion_types(potion_types: Dict[str, str]):
    ...
    with click.progressbar(potion_types) as potion_types_objects:
        ...
        for potion_type in potion_types_objects:
            try:
                effects = [
                    Effect.objects.get(name=effect) for effect in potion_type["effects"]
                ]
                p_type = PotionType(
                    name=potion_type["name"],
                    effects=effects,
                    description=potion_type["description"],
                )
                p_type.save()
            except DoesNotExist:
        ...
        click.echo(
            f"saved {len(potion_types) - len(unsaved_potion_types)} out of {len(potion_types)} potion types."
        )
...
@click.option(
    "-c",
    "--config-file",
    help="The JSON config file containing the potion types to create.",
)
@click.option(
    "-u",
    "--db-url",

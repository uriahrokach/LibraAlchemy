...
from backend.tests.app.api.response_models.potions import COUNTER_POTION, LAST_WATCH_POTION, WEAKNESS_POISON_POTION, \
    TRUTH_SYRUP_POTION,  HEALTH_POTION
...
@pytest.mark.parametrize("materials,technic,expected", [
    (
        ['לילית שחורה', 'קשקשי לטאת ענק', 'דרדר דרקון'],
        'בישול',
        [COUNTER_POTION]
    ),
    (
...
def test_brew_potion(materials, technic, expected):
    with potion_sandbox(
        ...
        res = requests.post(f'{ENDPOINT}/brew', json={
            'materials': materials,
            'technic': technic
        })
        print(res.json())
    assert sorted([potion['name'] for potion in res.json()]) == sorted([potion['name'] for potion in expected])


@pytest.mark.parametrize("materials,technic", [
    (
        ['קשקשי לטאת ענק', 'דרדר דרקון'],
        'בישול'
    ),
    (
        ['לילית שחורה', 'שורש אלף אצילי', 'קשקשי לטאת ענק', 'לוטוס דם', 'דרדר דרקון', 'פטריות מעמקים', 'תפרחת השעווה'],
        'התססה'
    )
])

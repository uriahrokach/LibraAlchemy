...
from backend.tests.app.api.response_models.potions import COUNTER_POTION, LAST_WATCH_POTION, WEAKNESS_POISON_POTION, \
    TRUTH_SYRUP_POTION, HEALTH_POTION, FAILED_POTION_VALIDATE_MATERIAL, FAILED_POTION_VALIDATE_TECHNIC, \
    FAILED_POTION_TO_HIGH_MATERIAL, FAILED_POTION_TO_LOW_MATERIAL
...
@pytest.mark.parametrize("request_json,expected_details", [
...
def test_material_number_failed_create_potion(request_json):
    res = requests.put(f'{ENDPOINT}/potion', json=request_json)
    assert res.status_code == 400 and res.json()['detail'] == f'Number of materials must be between 3 and 6'
...
def test_get_potion(name, expected_json):
    ...
    assert res.status_code == 200 and res.json()['description'] == expected_json['description']
...
@pytest.mark.parametrize("regex,expected_potions", [
    ('רעל', [COUNTER_POTION, WEAKNESS_POISON_POTION]),
    ('א', [LAST_WATCH_POTION, TRUTH_SYRUP_POTION, HEALTH_POTION]),
    ('ו', [COUNTER_POTION, LAST_WATCH_POTION, WEAKNESS_POISON_POTION, HEALTH_POTION]),
    ('fake', []),
    ('', [COUNTER_POTION, LAST_WATCH_POTION, WEAKNESS_POISON_POTION, TRUTH_SYRUP_POTION, HEALTH_POTION])
])
def test_regex_potion(regex, expected_potions):
    res = requests.get(f'{ENDPOINT}/potion?regex={regex}',)
    assert sorted([potion['name'] for potion in res.json()]) == sorted([potion['name'] for potion in expected_potions])


def test_regex_potion_no_param():
    res = requests.get(f'{ENDPOINT}/potion',)
    print(res.json())
    assert sorted([potion['name'] for potion in res.json()]) == sorted([potion['name'] for potion in [
        COUNTER_POTION, LAST_WATCH_POTION, WEAKNESS_POISON_POTION, TRUTH_SYRUP_POTION, HEALTH_POTION
    ]])


@pytest.mark.parametrize("name,request_json", [
    ('נוגדן רעלים', COUNTER_POTION),
    ('אשמורת אחרונה', LAST_WATCH_POTION),
    ('רעל חולשה', WEAKNESS_POISON_POTION),
    ('סם אמת', TRUTH_SYRUP_POTION),
